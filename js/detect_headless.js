...
// Test for user agent
function testBrowserLite(resultBlock) {
  let agent = BrowserDetect.browser;

  writeToBlock(resultBlock, agent);
  return /headless/i.test(agent);
}

function testBrowserNormal(resultBlock) {
  var ua = detect.parse(navigator.userAgent);
  let agent = ua.browser.family;

  writeToBlock(resultBlock, agent);
  return /headless/i.test(agent);
}

function testCookie(resultBlock) {

  let agent = navigator.cookieEnabled;
...
function testResolution(resultBlock) {

  let agent = screen.width + ' x ' + screen.height;

  writeToBlock(resultBlock, agent);
  return /headless/i.test(agent);
}

function testWebGL(resultBlock) {

  let agent = feature.webGL == feature.css3Dtransform;

  writeToBlock(resultBlock, agent);
  return /headless/i.test(agent);
}

function testBrowserSerious(resultBlock) {

var isOpera = (!!window.opr && !!opr.addons) || !!window.opera || navigator.userAgent.indexOf(' OPR/') >= 0;
if (isOpera) {
	writeToBlock(resultBlock, "Opera");
    return /headless/i.test( "Opera");
}
var isFirefox = typeof InstallTrigger !== 'undefined';
if (isFirefox) {
	writeToBlock(resultBlock, "Firefox");
    return /headless/i.test( "Firefox");
}
var isSafari = /constructor/i.test(window.HTMLElement) || (function (p) { return p.toString() === "[object SafariRemoteNotification]"; })(!window['safari'] || (typeof safari !== 'undefined' && safari.pushNotification));
if (isSafari) {
	writeToBlock(resultBlock, "Safari");
    return /headless/i.test("Safari");
}
var isIE = /*@cc_on!@*/false || !!document.documentMode;
if (isIE) {
	writeToBlock(resultBlock, "Internet Explorer");
    return /headless/i.test( "Internet Explorer ");
}
var isEdge = !isIE && !!window.StyleMedia;
if (isEdge) {
	writeToBlock(resultBlock, "Edge");
    return /headless/i.test( "Edge");
}
var isEdgeChromium = isChrome && (navigator.userAgent.indexOf("Edg") != -1);
if (isEdgeChromium) {
	writeToBlock(resultBlock, "Chromium");
    return /headless/i.test( "Chromium");
}
var isChrome = !!window.chrome;
if (isChrome) {
	writeToBlock(resultBlock, "Chrome");
    return /headless/i.test( "Chrome");
}

var isBlink = (isChrome || isOpera) && !!window.CSS;

if (isBlink) {
	writeToBlock(resultBlock, "Blink");
    return /headless/i.test( "Blink");
}
}
...
const tests = [
  ...
  { name: "Browser Simple",    id: "browser",           testFunction: testBrowserLite      },
  { name: "Browser Advanced",  id: "browser2",         testFunction: testBrowserNormal     },
  { name: "Browser Real",  id: "browser3",         testFunction: testBrowserSerious        },
  ...
  { name: "Cookie",            id: "cookie",            testFunction: testCookie           },
  { name: "Screen Resolution", id: "resolution",     testFunction: testResolution          },
  { name: "WebGL",             id: "webgl",        testFunction: testWebGL                 },
]
